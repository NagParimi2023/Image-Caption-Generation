...
#mapping of words to integer indices, and this layer will provide the reverse mapping from integer indices to words.
...
#Stabilizing Training: It helps stabilize the training process by mitigating the vanishing gradient problem.
#Facilitating Learning: By normalizing the input before further processing, the network is better able to learn complex patterns and relationships in the data.
...
#the model focuses on learning what should be added (or subtracted) to x to obtain the final output. 
...
class Embeddings(tf.keras.layers.Layer):
    ...
    def __init__(self, vocab_size, embed_dim, max_len):
        ...
        # Position embeddings are essential in transformer models to capture the sequential order of tokens.
        self.position_embeddings = tf.keras.layers.Embedding(
            max_len, embed_dim, input_shape=(None, max_len))
    

    def call(self, input_ids):
        #This method defines how the layer processes its input during a forward pass through the network. input_ids are the token IDs of the input sequence.
        length = tf.shape(input_ids)[-1]
        position_ids = tf.range(start=0, limit=length, delta=1)
        #Expands the dimensions of position_ids to make it compatible with the shape of input_ids.
        position_ids = tf.expand_dims(position_ids, axis=0)

        token_embeddings = self.token_embeddings(input_ids)
        position_embeddings = self.position_embeddings(position_ids)
   #Combines the token embeddings and position embeddings by element-wise addition.
...
class TransformerDecoderLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, embed_dim, units, num_heads):
        ...
        #Self-attention is often used in the decoder to model autoregressive relationships, where the generation of each token depends on the previously generated tokens in the sequence.
...
class ImageCaptioningModel(tf.keras.Model):
    ...
    def __init__(self, cnn_model, encoder, decoder, image_aug=None):
     ...
     #calculate_loss and calculate_accuracy are helper methods used to compute the loss and accuracy, respectively.
